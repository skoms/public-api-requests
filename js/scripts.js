	...
	[ ...document.querySelectorAll( '.modal-next' ) ].forEach( nextButton => {
		nextButton.addEventListener( 'click', e => {
			...
			if ( index !== 11 ) {
				...
				modalWindow.classList.remove( 'show' );
				modalWindow.classList.add( 'hide' );
				const nextModalWindow = document.querySelector( `.mod-${index + 1}` );
				nextModalWindow.classList.remove( 'hide' );
				nextModalWindow.classList.add( 'show' );
			}
		} )
	} );
}
...
function addSearchEventHandlers() {
	...
	searchInput.addEventListener( 'keyup', e => {
		const matches = employeeNames.filter( employeeName => employeeName.textContent.toLowerCase()
